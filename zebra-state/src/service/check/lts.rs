...
pub(crate) fn check_claimed_lts_payout(
    ...
    // The LTS contextual check only applies inside the disbursement window.
    // Outside the window — including all pre-NU7 heights, networks where NU7
    // isn't configured, and the pre-disbursement portion of NU7 — the
    // expected claim is zero, but the semantic verifier already enforces
    // strict transparent conservation (NU6 onward) or the historical
    // pre-NU6 inequality, so the implied-claim derivation against pre-NSM
    // blocks would fight that math. Skip cleanly here.
...
pub(crate) fn block_lts_pool_delta(
    ...
    let expected = expected_lts_payout_for(network, height, parent_lts_pool);
...
pub(crate) fn expected_lts_payout(
    ...
    match lts_disbursement_start(network) {
        Some(start) if height >= start => {
            // The parent pool snapshot is the LTS pool *after* the block at
            // `height - 1`. height ≥ disbursement_start ≥ NU7 activation > 0,
            // so the parent height is always available.
            ...
            lts_payout(height, network, parent_pool)
        }
        _ => Amount::zero(),
    }
}

/// `expected_lts_payout` variant that takes the parent pool directly,
/// without needing a `Chain`/`ZebraDb` lookup. Used by the finalized commit
/// path where the parent value pool is already in scope.
fn expected_lts_payout_for(
    network: &Network,
    height: Height,
    parent_lts_pool: Amount<NonNegative>,
) -> Amount<NonNegative> {
    match lts_disbursement_start(network) {
        Some(start) if height >= start => lts_payout(height, network, parent_lts_pool),
        _ => Amount::zero(),
    }
}
...
fn derive_implied_lts_claim(
    ...
    let deferred_pool_balance_change_nn = funding_streams
        ...
        .unwrap_or_default();
    let deferred_pool_balance_change: Amount<NegativeAllowed> =
        if Some(height) == NetworkUpgrade::Nu6_1.activation_height(network) {
            deferred_pool_balance_change_nn
                .checked_sub(network.lockbox_disbursement_total_amount(height))
                .expect(
                    "contextual LTS check: NU6.1 lockbox disbursement total fits in i64 for \
...
///
/// Panics if neither the chain nor the finalized state has block info at
/// `height`. This is a contextual invariant: every height ≤ tip has a
/// `BlockInfo` record (after the v28 disk-format upgrade), and we only call
...
fn resolve_lts_pool_at(
    ...
    let info = parent_chain
        ...
        .unwrap_or_else(|| {
            panic!(
                "LTS contextual check: no BlockInfo for ancestor height {height:?} \
                 in non-finalized chain or finalized state"
            )
        });

    info.value_pools().lts_amount()
}
